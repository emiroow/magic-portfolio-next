...
const AuthPage = () => {
  ...
  return (
    <div className="w-full relative flex min-h-screen items-center justify-center">
      ...
      <button onClick={() => back()} className="sm:hidden absolute top-5 left-4">
        <IoIosArrowBack className="text-2xl" />
      </button>

      <BlurFade className="w-full flex justify-center items-center" delay={BLUR_FADE_DELAY}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="hidden md:flex flex-col items-center justify-center gap-4 p-8 bg-gradient-to-br from-primary/10 via-accent/10 to-muted/10 border-r border-border/50">
              <div className="text-center space-y-2">
                ...
                <CardDescription className="text-sm max-w-sm mx-auto">{t('subtitle')}</CardDescription>
              </div>
            </div>
            ...
            <div className="px-4 sm:px-6 py-6 sm:py-8">
              <div className="md:hidden mb-4 text-center space-y-1.5">
                <CardTitle className="text-xl sm:text-2xl font-extrabold leading-tight tracking-tight">{t('title')}</CardTitle>
                <CardDescription className="mx-auto max-w-xs text-xs sm:text-sm text-muted-foreground">{t('subtitle')}</CardDescription>
              </div>

              <BlurFade delay={BLUR_FADE_DELAY * 4}>
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  {errors?.root?.message && <div className="rounded-md border px-3 py-2 text-sm text-destructive">{errors.root.message as any}</div>}
                    ...
                    <div>
                      <div className="relative">
                        <Input
                          ...
                          disabled={isPending}
                          className={`h-11 rounded-lg ${locale === 'fa' ? 'pl-12' : 'pr-12'}`}
                  ...
                  <div className="flex items-center justify-between">
                    <label htmlFor="remember" className="flex cursor-pointer select-none items-center gap-2 text-sm">
                      <input
                        type="checkbox"
                        id="remember"
                        checked={rememberMe}
                  ...
                  <Button type="submit" className="w-full rounded-lg py-3" disabled={isPending} size="lg">
                    {isPending ? (
